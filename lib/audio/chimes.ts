...
/** The mic was toggled on/off — one short blip, pitched by direction. */
export function playMicToggleBlip(micOn: boolean): void {
  playTones([{ freq: micOn ? 720 : 460, startAt: 0, duration: 0.06, gain: 0.06 }]);
}

/** Barge-in — the user interrupted Hugo mid-response. */
export function playBargeInBlip(): void {
  playTones([
